# Research: jansu-sans-io Crate

## Overview

`jansu-sans-io` is the Kafka wire protocol implementation at the heart of Jansu. As its name implies, it performs **no I/O** — it operates purely on bytes, converting between raw Kafka protocol frames and typed Rust structures. This makes it suitable for embedding in any async runtime or network stack (Jansu uses `rama`).
...
2. **Parse**: Converts JSON values into `jansu_model::Message` structs (which model version ranges, field metadata, nested structures)
...
- `jansu-model` — Kafka message metadata model (`MessageMeta`, `FieldMeta`, `Message`)
...
- `jansu-model` — Parses Kafka JSON descriptors into structured metadata
