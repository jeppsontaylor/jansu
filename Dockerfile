...
RUN xx-cargo build --bin jansu --all-features --release --target-dir ./build
RUN xx-verify --static ./build/$(xx-cargo --print-target-triple)/release/jansu

RUN <<EOF
mkdir -p /image/schema /image/data /image/tmp /image/etc/ssl
cp -v build/$(xx-cargo --print-target-triple)/release/jansu /image
cp -v LICENSE /image
