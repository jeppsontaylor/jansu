...
For phase work, audit work, or prompts like "progress on MASTER_PLAN and audit", read `AGENTS.md` first. Then read `MASTER_PLAN.md`, `AUDIT.md`, `phase-logs/index.json`, and the relevant `tips/phases` document.

All phase-backed work must update `phase-logs`, and compatibility claims must point to `docs/compatibility/kafka-4.2-ledger.json` plus test proof.

## Project Overview

Jansu is a stateless, Apache Kafka-compatible broker written in Rust. It is a drop-in replacement for Apache Kafka with pluggable storage backends: PostgreSQL, libSQL (SQLite), S3/object store, and memory. Schema-backed topics (Avro, JSON Schema, Protocol Buffers) can be written as Apache Iceberg or Delta Lake tables.
...
Note: when running jansu directly (not via docker compose), set `AWS_ENDPOINT="http://localhost:9000"` in `.env`.

## Architecture

Cargo workspace with 15 member crates, producing a single binary (`jansu`) with subcommands: `broker` (default), `cat`, `topic`, `generator`, `perf`, `proxy`.

### Key Crates

| Crate | Role |
|-------|------|
| `jansu` | Binary entry point, subcommand dispatch |
| `jansu-broker` | Kafka API broker: `Broker<G, S>` generic over Coordinator + Storage |
| `jansu-sans-io` | **Code-generated** Kafka wire protocol (pure serde, no I/O) |
| `jansu-service` | Network service layers built on `rama` (Layer/Service composition) |
| `jansu-storage` | Storage abstraction: `StorageContainer` enum over backends |
| `jansu-schema` | Schema registry + Iceberg/Delta/Parquet lake integration |
| `jansu-client` | Async Kafka protocol client (rama service layers) |
| `jansu-model` | Kafka JSON protocol definitions (used in build.rs) |
| `jansu-cat` | CLI: produce/consume Avro, JSON, Protobuf messages |
| `jansu-cli` | Clap-based CLI argument parsing |

### Sans-I/O Code Generation (`jansu-sans-io`)

`jansu-sans-io/build.rs` reads ~185 official Kafka JSON message descriptors from `jansu-sans-io/message/*.json` and generates typed Rust structs for every request/response pair. **Do not manually edit generated files.** The message JSON files are from upstream Apache Kafka.

### Service Layer Pattern (`jansu-service`)
...
### Storage Backends (`jansu-storage`)
...
- Group coordination in `jansu-broker/src/coordinator/group/`
...
- Tests in `jansu-broker` run against multiple backends: InMemory, Lite (libSQL), Postgres, SlateDb
...
| `jansu-sans-io/message/` | Kafka JSON protocol descriptors (upstream, ~185 files) |
| `jansu-sans-io/build.rs` | Code generator: JSON descriptors -> Rust types |
